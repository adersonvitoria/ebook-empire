...
**Login do dashboard (agir pela tela) — FUNCIONAL.** `POST /auth/login` (`apps/api/src/routes/auth.ts`) recebe `{ password }`, compara contra `ADMIN_PASSWORD` em tempo constante (`timingSafeEqual`) e devolve `{ token, expiresInSec }` (JWT `{ role:'admin', sub:'admin' }`, TTL `AUTH_TOKEN_TTL_SEC` default 12h); senha errada => 401, `ADMIN_PASSWORD` vazia => 503 `login_disabled`. `GET /auth/me` valida o Bearer. As rotas/guards admin existentes **nao** mudaram (so foi adicionado `/auth`). Frontend: `AuthProvider`/`useAuth` (token em `localStorage`), barra de login (`components/auth-bar.tsx`), `lib/api.ts` anexa `Authorization: Bearer`, e os botoes de acao em 8+ paginas chamam a API autenticada. CORS ja permite `Authorization`. Verificacao: typecheck 5/5; `next build` PASS; e2e dedicado de auth (`e2e:auth`) 13/13; os 5 e2e de regressao verdes. **Setar `ADMIN_PASSWORD` em producao (Railway) e o unico passo manual para destravar as acoes** — ver `RUNBOOK.md` §0 e `DEPLOY.md` §5.
...
| **apps/web** (Dashboard Next.js 14) | FUNCIONAL (UI) — **agir pela tela habilitado** | nao (login exige `ADMIN_PASSWORD` na API) | Overview, ebooks, orders, social, ads, agents + Command Center. **Login admin** (single-admin): barra de auth no topo, token JWT em `localStorage`, `Authorization: Bearer` anexado a todas as requisicoes; paginas continuam VISIVEIS sem login (GET publicos) e os botoes de acao (kill switch, aprovar/rejeitar/rollback, scan, gerar ebook, salvar guardrails/alertas, etc.) ficam habilitados apos logar. **Sem runner de testes** (sem vitest no `apps/web`; validado por `tsc --noEmit` + `next build`). Ver `RUNBOOK.md` §0. |
